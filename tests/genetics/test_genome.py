...
from biomics.genetics import ProkaryoticGenome, DNASequence
...
@pytest.mark.parametrize(
    "genome, ori_center, window_size, expected",
    [
        ("CATGGGCATCGGCCATACGCC", 8, 10, "GGGCATCGGCC"),
        ("CATGGGCATCGGCCATACGCC", 2, 10, "GCCCATGGGCA"),
    ],
)
def test_get_region_in_circular_array(genome, ori_center, window_size, expected):
    genome = ProkaryoticGenome(genome)
    assert (
        genome._get_region_in_circular_array(center=ori_center, window_size=window_size)
        == expected
    )
